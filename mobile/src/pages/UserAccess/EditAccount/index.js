...
export const EditAccount = ({route}) => {
    ...
    return (
        <View style={{flex: 1, justifyContent: 'space-between', alignItems: 'center', padding: 20, paddingVertical: 20}}>
            ...
            {type === 'nome' ? (
                <Input value={name} onChange={(e) => setName(e)} style={{marginTop: 10, fontFamily: 'Poppins_400Regular'}} ph="Nome completo"/>
            ) : type === 'telefone' ? (
                <MaskedInputPhone onChange={(e) => setPhone(e)} value={phone} ph="(00) 00000-0000" style={{fontFamily: 'Poppins_400Regular'}}/>
            ) : (
                <Input value={email} onChange={(e) => setEmail(e)} style={{marginTop: 10, fontFamily: 'Poppins_400Regular'}} ph="mariadasilva@email.com"/>
            )} 
